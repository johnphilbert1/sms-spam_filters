...
class SpamFragment : Fragment() {
    ...
    private fun setupViews(view: View) {
        ...
        searchEditText.setOnEditorActionListener { _, _, _ ->
            viewModel.searchSpamMessages(searchEditText.text.toString())
            true
        }
    }
    ...
    private fun showContextMenu(message: Message) {
        val options = arrayOf("Not Spam", "Delete")
        
        AlertDialog.Builder(requireContext())
            .setTitle("Message Actions")
            .setItems(options) { _, which ->
                when (which) {
                    0 -> markAsNotSpam(message)
    ...
    private fun markAsNotSpam(message: Message) {
        AlertDialog.Builder(requireContext())
            .setTitle("Mark as Not Spam")
            .setMessage("Are you sure you want to mark this message as not spam?")
            .setPositiveButton("Yes") { _, _ ->
                viewModel.restoreMessage(message)
                viewModel.trainHam(message)
                android.widget.Toast.makeText(requireContext(), "Marked as not spam", android.widget.Toast.LENGTH_SHORT).show()
            }
    ...
    private fun observeData() {
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.spamMessages.collectLatest { messages ->
